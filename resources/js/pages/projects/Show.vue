...
import { edit, destroy, index } from '@/actions/App/Http/Controllers/ProjectController';
...
const totalInvoiceAmount = computed(() => {
  return props.invoices.reduce((sum: number, invoice: Invoice) => sum + invoice.amount, 0)
})
        ...
        >
          <div class="space-y-1">
            ...
            <p class="text-sm text-muted-foreground sm:text-base">
              Project created {{ formatDate(props.project.created_at) }}
              <span v-if="props.project.client" class="ml-2">• {{ props.project.client.company }}</span>
            </p>
          </div>
          ...
          >
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger as-child>
                  <Link :href="createTask.url() + `?project_id=${props.project.id}`">
                    <Button
                      variant="outline"
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger as-child>
                  <Link :href="createInvoice.url() + `?project_id=${props.project.id}`">
                    <Button
                      variant="outline"
                      class="h-9 w-9 p-0 md:px-4 md:py-2 lg:h-auto lg:w-auto"
                      size="sm"
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <CardContent class="space-y-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1">
                  ...
                  <Badge :class="getStatusColor(props.project.status)" class="capitalize">
                ...
                <div class="space-y-1">
                  ...
                  <p class="text-sm flex items-center gap-2">
                    ...
                    <span v-if="props.project.end_date && getDaysRemaining(props.project.end_date) !== null" 
                          :class="{
                            'text-red-600': getProgressStatus(props.project.end_date) === 'overdue',
                            'text-orange-600': getProgressStatus(props.project.end_date) === 'urgent',
                            'text-yellow-600': getProgressStatus(props.project.end_date) === 'warning',
                            'text-muted-foreground': getProgressStatus(props.project.end_date) === 'info'
                          }"
                          class="text-xs font-medium">
                      ({{ getDaysRemaining(props.project.end_date)! < 0 ? 'Overdue' : `${getDaysRemaining(props.project.end_date)} days left` }})
                    </span>
                  </p>
                </div>
              </div>
              ...
              <div class="space-y-1">
                ...
                <p class="text-sm whitespace-pre-wrap">{{ props.project.description || 'No description provided' }}</p>
              </div>
              ...
              <div v-if="props.project.client" class="space-y-1">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <Link :href="`/clients/${props.project.client.id}`" class="text-primary hover:underline text-sm">
                    {{ props.project.client.name }} - {{ props.project.client.company }}
                  </Link>
                </div>
              </div>
              ...
              <div class="space-y-3">
                ...
                <div v-if="props.project.members.length > 0" class="flex flex-wrap gap-2">
                  <div v-for="member in props.project.members" :key="member.id" 
                       class="flex items-center gap-2 px-3 py-2 border rounded-lg bg-muted/5">
                ...
                <p v-else class="text-sm text-muted-foreground">No team members assigned</p>
              </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t">
                <div class="space-y-1">
                  ...
                  <p class="text-sm">{{ props.project.creator?.name || 'N/A' }}</p>
                </div>
                
                <div class="space-y-1">
                  <Label class="text-sm font-medium text-muted-foreground">Last Updated</Label>
                  <p class="text-sm">{{ formatDate(props.project.updated_at) }}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card class="border">
            ...
            <CardContent>
              <div v-if="props.tasks.length > 0" class="space-y-3">
                <div 
                  ...
                  <div class="flex justify-between items-start">
                    <div class="flex-1">
                      ...
                      <div class="flex items-center gap-2 mt-1">
                        <Badge :class="getTaskStatusColor(task.status)" class="capitalize text-xs">
                          {{ task.status.replace('_', ' ') }}
                        </Badge>
                        <Badge :class="getPriorityColor(task.priority)" class="capitalize text-xs">
                          {{ task.priority }}
                        </Badge>
              ...
              <div v-else class="text-center py-8">
                <CheckCircle class="h-12 w-12 text-muted-foreground mx-auto mb-3" />
                  ...
                  <div class="flex justify-between items-start">
                    <div class="flex-1">
                      ...
                      <div class="flex items-center gap-2 mt-1">
                        ...
                        <span v-if="invoice.due_date && getDaysRemaining(invoice.due_date) !== null && getDaysRemaining(invoice.due_date)! < 0 && invoice.status !== 'paid'" 
          ...
          <Card class="border">
            ...
            <CardContent class="space-y-4">
              ...
              <div v-else class="text-center py-8">
                ...
                <p class="text-sm text-muted-foreground mt-1">
                  Add the first note to track important project information
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card class="border">
            ...
            <CardContent class="space-y-4">
              <div class="grid grid-cols-2 gap-4">
                ...
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p>Total invoices for this project</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger as-child>
                      <div class="text-center p-4 border rounded-lg bg-muted/5 cursor-default">
                        <div class="text-2xl font-bold text-primary">{{ props.project.members.length }}</div>
                        <div class="text-sm text-muted-foreground mt-1">Members</div>
                      </div>
                    </TooltipTrigger>
                    <TooltipContent>
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger as-child>
                      <div class="text-center p-4 border rounded-lg bg-muted/5 cursor-default">
                        <div class="text-2xl font-bold text-primary">{{ props.documents_count }}</div>
                        <div class="text-sm text-muted-foreground mt-1">Documents</div>
                      </div>
                    </TooltipTrigger>
                    <TooltipContent>
                      <p>Total documents associated with this project</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              </div>
              ...
              <div class="pt-4 border-t">
                <h4 class="text-sm font-medium mb-3">Financial Summary</h4>
                <div class="space-y-2">
                  <div class="flex justify-between items-center">
                    <span class="text-sm text-muted-foreground">Total Invoiced</span>
                    <!-- FIXED: Now using the computed property -->
                    <span class="text-sm font-medium">{{ formatCurrency(totalInvoiceAmount) }}</span>
                  </div>
                  <div class="flex justify-between items-center">
                    <span class="text-sm text-muted-foreground">Project Budget</span>
                    <span class="text-sm font-medium">{{ formatCurrency(props.project.budget) }}</span>
                  </div>
                </div>
              </div>
            </CardContent>
